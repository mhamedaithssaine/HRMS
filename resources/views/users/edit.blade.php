<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('users.update', $user->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label for="password" class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Mot de Passe (laisser vide pour ne pas changer):</label>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" name="recrut_date" id="recrut_date" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('recrut_date', $user->recrut_date) }}">
                            @error('recrut_date')
                                <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="contract_id" id="contract_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:shadow-outline">
                                ...
                                @foreach($contracts as $contract)
                                    <option value="{{ $contract->id }}" {{ old('contract_id', $user->contract_id) == $contract->id ? 'selected' : '' }}>{{ $contract->type }}</option>
                                @endforeach
                            </select>
                            @error('contract_id')
                                <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (initialDepartmentId && emploisByDepartment[initialDepartmentId]) {
                emploisByDepartment[initialDepartmentId].forEach(emploi => {
